...
function Card({ scan, bySet }: { scan: Scan; bySet?: boolean }) {
  const titles = scan.poems.map((p) => p.title_vi || p.title || "Không đề").join(" · ");
  ...
  return (
    <Link href={`/poems/${scan.scanId}`} className="pcard">
      <div className="pttl">{titles}</div>
      <div className="pmt">
        {sub && <span>{sub}</span>}
  ...
  return (
    <>
      <header className="g-header">
        <h1>Ông Nội Poetry</h1>
        <div className="g-sub">
          {scans.length} pages · {total} poems · <Link href="/admin/suggestions">admin ›</Link>
        </div>
      </header>
